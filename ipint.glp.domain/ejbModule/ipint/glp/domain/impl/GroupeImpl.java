...
public class GroupeImpl implements GroupeService {
	...
	public GroupeDTO creer(GroupeDTO obj) throws MetierException {
		...
		if (obj.getUtilisateurResponsable() == null) {
			throw new InformationManquanteException(
					"GroupeImpl.creer : Le groupeDTO n'a pas d'utilisateur responsable");
		}
		if (obj.getUtilisateurResponsable().getIdUtilisateur() == null
				&& obj.getUtilisateurResponsable().getEmail() == null) {
			throw new InformationManquanteException(
					"GroupeImpl.creer : L'utilisateur responsable du groupeDTO n'a ni id ni email");
		}
		...
		if (!q.getResultList().isEmpty()) {
			throw new GroupeExistantException(
					"GroupeImpl.creer : Un groupe avec le nom " + obj.getNomGroupe() + " existe déjà");
		}
		...
		if (obj.getUtilisateurResponsable().getIdUtilisateur() != null) {
			...
			if (utilisateur == null) {
				throw new UtilisateurInconnuException("GroupeImpl.creer : " + obj.getUtilisateurResponsable().toString()
						+ " n'existe pas pour cet id");
			}
		...
		// if(article==null){
		// throw new ArticleInconnuException("GroupeImpl.creer :
		// "+articleDto.toString()+" n'existe pas");
		// }
		...
		// if(u==null){
		// throw new UtilisateurInconnuException("GroupeImpl.creer :
		// "+utilisateurDto.toString()+" n'existe pas pour cet id");
		// }
		// utilisateurs.add(u);
		// } else if (utilisateurDto.getEmail() != null) {
		// q = em.createQuery(
		// "select u from Utilisateur u where u.email = '" +
		// utilisateurDto.getEmail() + "'");
		// try{
		// u = (Utilisateur) q.getSingleResult();
		// }catch(NoResultException e){
		// throw new UtilisateurInconnuException("GroupeImpl.creer :
		// "+utilisateurDto.toString()+" n'existe pas pour cet email");
		// }
		// utilisateurs.add(u);
		// // + gerer la bidirection
		// }
			...
			if (utilisateurDto.getIdUtilisateur() != null) {
				...
				if (u == null) {
					throw new UtilisateurInconnuException(
							"GroupeImpl.creer : " + utilisateurDto.toString() + " n'existe pas pour cet id");
				}
				animateurs.add(u);
			} else if (utilisateurDto.getEmail() != null) {
				q = em.createQuery("select u from Utilisateur u where u.email = :email");
				...
				try {
					...
					throw new UtilisateurInconnuException(
							"GroupeImpl.creer : " + utilisateurDto.toString() + " n'existe pas pour cet email");
				}
				animateurs.add(u);
			}
			if (u != null) {
		...
		q = em.createQuery("select u from UtilisateurGroupes u where u.email = :email");
		...
		if (obj.getIdGroupe() != null) {
			...
			if (gr == null) {
				throw new GroupeInconnuException(
						"GroupeImpl.trouver : " + obj.toString() + " n'existe pas pour cet id");
			}
		} else {
			...
			try {
				...
				throw new GroupeInconnuException(
						"GroupeImpl.trouver : " + obj.toString() + " n'existe pas pour ce nom de groupe");
			}
			// Query q2 = em.createQuery("select a from Article a where
			// a.groupe_idgroupe = '" + grp.getIdGroupe() + "'");
	...
	public GroupeDTO modifier(GroupeDTO ancienObj, GroupeDTO nouvelObj) throws MetierException {
		...
		if (ancienObj.getIdGroupe() != null) {
			...
			if (groupeMAJ == null) {
				throw new GroupeInconnuException(
						"GroupeImpl.modifier : " + ancienObj.toString() + " est inconnu pour cet id");
			}
		} else {
			q = em.createQuery("select g from Groupe g where g.nomGroupe = :nom");
			...
			try {
				...
				throw new GroupeInconnuException(
						"GroupeImpl.trouver : " + ancienObj.toString() + " n'existe pas pour ce nom de groupe");
			}
		}
		...
		// nouvelObj.getUtilisateurResponsable().getIdUtilisateur());
		// if(utilisateur==null){
		// throw new UtilisateurInconnuException("GroupeImpl.modifier :
		// "+nouvelObj.getUtilisateurResponsable().toString()+" n'existe pas
		// pour cet id");
		// }
		...
		// q = em.createQuery(
		// "select u from Utilisateur u where u.email = '" +
		// nouvelObj.getUtilisateurResponsable().getEmail() + "'");
		// try{
		// utilisateur = (Utilisateur) q.getSingleResult();
		// }catch(NoResultException e){
		// throw new UtilisateurInconnuException("GroupeImpl.modifier :
		...
		// }
		//
		// utilisateur.getGroupesGeres().remove(groupeMAJ);//le equals est sur l
		// id et le nom du groupe
		// em.persist(utilisateur);
		//
	...
	@Override
	public GroupeDTO ajouterUtilisateur(GroupeDTO groupeDTOAModifier, UtilisateurDTO utilisateurDTOAAjouter)
		...
		if (groupeDTOAModifier.getIdGroupe() == null && groupeDTOAModifier.getNomGroupe() == null) {
			throw new InformationManquanteException(
					"GroupeImpl.ajouterUtilisateur : Le groupe à modifier n'a ni id ni nom de groupe");
		}
		...
		if (utilisateurDTOAAjouter.getIdUtilisateur() != null) {
			...
			if (utilisateurAAjouter == null) {
				throw new UtilisateurInconnuException("GroupeImpl.ajouterUtilisateur : "
						+ utilisateurDTOAAjouter.toString() + " n'existe pas pour cet id");
			}
			...
			try {
				...
				throw new UtilisateurInconnuException("GroupeImpl.ajouterUtilisateur : "
						+ utilisateurDTOAAjouter.toString() + " n'existe pas pour cet email");
			}
		}
		...
		if (groupeDTOAModifier.getIdGroupe() != null) {
			...
			if (groupeAModifier == null) {
				throw new GroupeInconnuException("GroupeImpl.ajouterUtilisateur : " + groupeDTOAModifier.toString()
						+ " n'existe pas pour cet id");
			}
		} else {
			q = em.createQuery("select g from Groupe g where g.nomGroupe = :nom");
	...
	@Override
	public GroupeDTO supprimerUtilisateur(GroupeDTO groupeDTOAModifier, UtilisateurDTO utilisateurDTOASuppimer)
			throws MetierException {
		if (groupeDTOAModifier == null) {
		...
		if (utilisateurDTOASuppimer == null) {
			throw new InformationManquanteException(
					"GroupeImpl.supprimerUtilisateur : L'utilisateur à supprimer est null");
		}
		if (groupeDTOAModifier.getIdGroupe() == null && groupeDTOAModifier.getNomGroupe() == null) {
			throw new InformationManquanteException(
					"GroupeImpl.supprimerUtilisateur : Le groupe à modifier n'a ni id ni nom de groupe");
		}
		if (utilisateurDTOASuppimer.getIdUtilisateur() == null && utilisateurDTOASuppimer.getEmail() == null) {
			throw new InformationManquanteException(
					"GroupeImpl.supprimerUtilisateur : L'utilisateur n'a ni id ni email");
		}
		...
		if (utilisateurDTOASuppimer.getIdUtilisateur() != null) {
			...
			if (utilisateurASupprimer == null) {
				throw new UtilisateurInconnuException("GroupeImpl.supprimerUtilisateur : "
						+ utilisateurDTOASuppimer.toString() + " n'existe pas pour cet id");
			}
			...
			try {
				...
				throw new UtilisateurInconnuException("GroupeImpl.supprimerUtilisateur : "
						+ utilisateurDTOASuppimer.toString() + " n'existe pas pour cet email");
			}
		}
		...
		if (groupeDTOAModifier.getIdGroupe() != null) {
			groupeAModifier = em.find(Groupe.class, groupeDTOAModifier.getIdGroupe());
			if (groupeAModifier == null) {
				throw new GroupeInconnuException("GroupeImpl.supprimerUtilisateur : " + groupeDTOAModifier.toString()
						+ " n'existe pas pour cet id");
			}
		} else {
			q = em.createQuery("select g from Groupe g where g.nomGroupe = :nom");
	...
	@Override
	public GroupeDTO ajouterAnimateur(GroupeDTO groupeDTOAModifier, UtilisateurDTO animateurDTOAAjouter)
			throws MetierException {
		if (groupeDTOAModifier == null) {
		...
		if (groupeDTOAModifier.getIdGroupe() == null && groupeDTOAModifier.getNomGroupe() == null) {
			throw new InformationManquanteException(
					"GroupeImpl.ajouterAnimateur : Le groupe à modifier n'a ni id ni nom de groupe");
		}
		...
		if (animateurDTOAAjouter.getIdUtilisateur() != null) {
			...
			if (animateurAAjouter == null) {
				throw new UtilisateurInconnuException("GroupeImpl.ajouterAnimateur : " + animateurDTOAAjouter.toString()
						+ " n'existe pas pour cet id");
			}
		...
		if (groupeDTOAModifier.getIdGroupe() != null) {
			...
			if (groupeAModifier == null) {
				throw new GroupeInconnuException(
						"GroupeImpl.ajouterAnimateur : " + groupeDTOAModifier.toString() + " n'existe pas pour cet id");
			}
		} else {
			q = em.createQuery("select g from Groupe g where g.nomGroupe = :nom");
			q.setParameter("nom", groupeDTOAModifier.getNomGroupe());
			try {
				...
				throw new GroupeInconnuException("GroupeImpl.ajouterAnimateur : " + groupeDTOAModifier.toString()
						+ " n'existe pas pour ce nom de groupe");
			}
		}
	...
	@Override
	public GroupeDTO supprimerAnimateur(GroupeDTO groupeDTOAModifier, UtilisateurDTO animateurDTOASupprimer)
			throws MetierException {
		if (groupeDTOAModifier == null) {
		...
		if (groupeDTOAModifier.getIdGroupe() == null && groupeDTOAModifier.getNomGroupe() == null) {
			throw new InformationManquanteException(
					"GroupeImpl.supprimerAnimateur : Le groupe à modifier n'a ni id ni nom de groupe");
		}
		...
		if (animateurDTOASupprimer.getIdUtilisateur() != null) {
			...
			if (animateurASupprimer == null) {
				throw new UtilisateurInconnuException("GroupeImpl.supprimerAnimateur : "
						+ animateurDTOASupprimer.toString() + " n'existe pas pour cet id");
			}
			...
			try {
				...
				throw new UtilisateurInconnuException("GroupeImpl.supprimerAnimateur : "
						+ animateurDTOASupprimer.toString() + " n'existe pas pour cet email");
			}
		}
		...
		if (groupeDTOAModifier.getIdGroupe() != null) {
			groupeAModifier = em.find(Groupe.class, groupeDTOAModifier.getIdGroupe());
			if (groupeAModifier == null) {
				throw new GroupeInconnuException("GroupeImpl.supprimerAnimateur : " + groupeDTOAModifier.toString()
						+ " n'existe pas pour cet id");
			}
		} else {
			q = em.createQuery("select g from Groupe g where g.nomGroupe = :nom");
	...
	@Override
	public GroupeDTO modifierResponsable(GroupeDTO groupeDTOAModifier, UtilisateurDTO nouveauResponsableDTO)
			throws MetierException {
		if (groupeDTOAModifier == null) {
		...
		if (nouveauResponsableDTO == null) {
			throw new InformationManquanteException(
					"GroupeImpl.modifierResponsable : Le responsable à ajouter est null");
		}
		if (groupeDTOAModifier.getIdGroupe() == null && groupeDTOAModifier.getNomGroupe() == null) {
			throw new InformationManquanteException(
					"GroupeImpl.modifierResponsable : Le groupe à modifier n'a ni id ni nom de groupe");
		}
		if (nouveauResponsableDTO.getIdUtilisateur() == null && nouveauResponsableDTO.getEmail() == null) {
			throw new InformationManquanteException(
					"GroupeImpl.modifierResponsable : Le responsable n'a ni id ni email");
		}
		...
		if (nouveauResponsableDTO.getIdUtilisateur() != null) {
			...
			if (nouveauResponsable == null) {
				throw new UtilisateurInconnuException("GroupeImpl.modifierResponsable : "
						+ nouveauResponsableDTO.toString() + " n'existe pas pour cet id");
			}
			...
			try {
				...
				throw new UtilisateurInconnuException("GroupeImpl.modifierResponsable : "
						+ nouveauResponsableDTO.toString() + " n'existe pas pour cet email");
			}
		}
		...
		if (groupeDTOAModifier.getIdGroupe() != null) {
			groupeAModifier = em.find(Groupe.class, groupeDTOAModifier.getIdGroupe());
			if (groupeAModifier == null) {
				throw new GroupeInconnuException("GroupeImpl.modifierResponsable : " + groupeDTOAModifier.toString()
						+ " n'existe pas pour cet id");
			}
		} else {
			q = em.createQuery("select g from Groupe g where g.nomGroupe = :nom");
	...
	public List<UtilisateurDTO> listerUtilisateurs(GroupeDTO groupeDTO) throws MetierException {
		...
		if (groupeDTO.getIdGroupe() == null && groupeDTO.getNomGroupe() == null) {
			throw new InformationManquanteException(
					"GroupeImpl.listerUtilisateurs : Le groupe n'a ni id ni nom de groupe");
		}
		...
		try {
			...
			if (groupeDTO.getIdGroupe() != null) {
				...
				throw new GroupeInconnuException(
						"Le groupe ayant pour nom " + groupeDTO.getNomGroupe() + " est inconnu");
			}
		}
